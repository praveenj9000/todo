...
export async function cancelListQuery(queryClient: QueryClient, queryKey: QueryKey) {
  await queryClient.cancelQueries({ queryKey });
}

export function getInfiniteCache<TPage, TPageParam>(queryClient: QueryClient, queryKey: QueryKey) {
...
export function setInfiniteCacheItems<TItem extends EntityWithId, TPage, TPageParam>(
  ...
  queryClient.setQueryData<InfiniteData<TPage, TPageParam>>(queryKey, (previous) => {
    if (!previous) {
      return {
    ...
    const pageSizes = previous.pages.map((page) => accessor.getItems(page).length);
    ...
    pageSizes.forEach((size, index) => {
      ...
      const slice = isLast ? items.slice(offset) : items.slice(offset, offset + size);
...
export function invalidateList(queryClient: QueryClient, queryKey: QueryKey) {
